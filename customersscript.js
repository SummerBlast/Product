...
function login() {
    ...
    // For simplicity, let's assume successful login and open the customer panel
    openPanel('customer');
}

// Function to open different panels based on the user type
function openPanel(userType) {
    // Hide login container
    document.querySelector('.login-container').style.display = 'none';

    // Show respective panel
    if (userType === 'customer') {
        document.querySelector('.side-navbar').classList.remove('hide');
    } else if (userType === 'seller') {
        // Implement seller panel
        // (You need to add HTML and JavaScript code for seller functionalities)
    } else if (userType === 'admin') {
        // Implement admin panel
        // (You need to add HTML and JavaScript code for admin functionalities)
    }
}
function signOut() {
